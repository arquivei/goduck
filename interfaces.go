...
// Queue represents a "pool" of unordered messages, where each one can be
// individually marked as complete.
type Queue interface {
	// Poll should return the next message available in the queue. If the
	...
	// appear on subsequent "Poll" calls.
	...
	// Close closes the Queue. After calling this function, "Poll" should
	// return io.EOF
	Close() error
}
...
type Stream interface {
	...
	// Close closes the Stream. After calling this function, "Poll" should
	// return io.EOF
	Close() error
}
